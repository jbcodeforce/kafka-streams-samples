package jbcodeforce.kafka.study;
...
/**
 * A bean producing random temperature data every second. The values are written
 * to a Kafka topic (temperature-values). Another topic contains the name of
 * weather stations (weather-stations). The Kafka configuration is specified in
 * the application configuration.
 * 
 * Producer use reactive messaging outgoing annotation to dispatch records to messaging backend.
 * Flowable emitting a random temperature value every 0.5 seconds
 * 
 * The two methods with outgoing annotations each return a reactive stream whose items are sent 
 * to the streams named temperature-values and weather-stations, respectively.
 * 
